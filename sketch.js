...
rex.setCollider("circle",0,0,30);
  ...
  //solo inv 
  solo_inv= createSprite(width/2,height-10,width,20);
  solo_inv.visible = false;
  
  finaldojogo = createSprite(width/2,height/2);
...
touches=[];      
  ...
  textSize (13);
  text("ponto = "+ ponto,width/2+250,height-500);
...
function gerarnuvin(){
  ...
  if (frameCount % 60 === 0){
   nuvem = createSprite(width,100,10,10);
...
if(frameCount % 80 === 0){
  var obstaculo = createSprite(width,height-35,10,10);
  
  var rand = Math.round(random(1,6));
  obstaculo.scale = 0.5;
  
  obstaculo.velocityX = -6;
  obstaculo.lifetime = width-120;
